...
// ==== AGENT MONGODB INIT ====
// Dedicated MongoDB connection for agent subsystems (memory, sessions, cron)
let agentMongoClient = null;

try {
  const agentMongoURL = process.env.MONGODB_URL || "mongodb://localhost:27017";
  agentMongoClient = new MongoClient(agentMongoURL, { serverSelectionTimeoutMS: 5000 });
  await agentMongoClient.connect();
  const agentDb = agentMongoClient.db(dbConfig.db || "dotBot");

  await initMemory(agentDb);
  await initSessions(agentDb);
  await initCron(agentDb, async (task) => {
    // When a cron task fires, inject it into the agent loop as a synthetic message
    ...
    try {
      ...
      const { saveSession } = await import("./agent/session.js");
      await saveSession(sessionId, session.messages, session.model);
    } catch (err) {
      console.error("[cron] agent loop error for task:", task.name, err.message);
    }
  });

  logger.info("Agent subsystems initialized (memory, sessions, cron)");
} catch (err) {
  logger.warn("Agent MongoDB init failed — agent features disabled", { error: err.message });
  agentMongoClient = null;
}
...
// ==== AGENT ROUTES ====
if (agentMongoClient) {
  const agentRoutes = createAgentRoutes(authMiddleware, csrfProtection);
  app.route("/api/agent", agentRoutes);
  logger.info("Agent routes mounted at /api/agent");
}
...
if (typeof process !== 'undefined') {
  const gracefulShutdown = async (signal) => {
    ...
    server.close(async () => {
      ...
      // Stop agent cron and close agent MongoDB connection
      try {
        stopCron();
        if (agentMongoClient) await agentMongoClient.close();
        console.log('Agent connections closed');
      } catch (err) {
        console.error('Error closing agent connections:', err);
      }
